...
def main(args: argparse.Namespace):
    ...
    args.animal = "horse"
    val_target_dataset_horse = datasets.__dict__[args.target](is_train=False, **vars(args))
    val_target_loader_horse = torch.utils.data.DataLoader(
        val_target_dataset_horse,
        batch_size=args.test_batch, shuffle=False,
        num_workers=args.workers
    )
    args.animal = "tiger"
    val_target_dataset_tiger = datasets.__dict__[args.target](is_train=False, **vars(args))
    val_target_loader_tiger = torch.utils.data.DataLoader(
        val_target_dataset_tiger,
        batch_size=args.test_batch, shuffle=False,
        num_workers=args.workers
    )
    ...
    if args.resume:
        ...
        stu_optimizer.load_state_dict(checkpoint['d_optimizer'])
        # tea_optimizer.load_state_dict(checkpoint['tea_optimizer'])
        lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
        lr_scheduler.load_state_dict(checkpoint['lr_dscheduler'])
    ...
    if args.phase == 'test':
        ...
        target_val_acc_h = validate(val_target_loader_horse, teacher, criterion, visualize, args)
        target_val_acc_t = validate(val_target_loader_tiger, teacher, criterion, visualize, args)
        logger.write("Source: {:4.3f} Target: {:4.3f} Horse: {:4.3f} Tiger: {:4.3f}".format(source_val_acc['all'], target_val_acc['all'], target_val_acc_h['all'], target_val_acc_t['all']))
        for name, acc in target_val_acc.items():
            logger.write("{}: {:4.3f}".format(name, acc))
        logger.write("Horse:")
        for name, acc in target_val_acc_h.items():
            logger.write("{}: {:4.3f}".format(name, acc))
        logger.write("Tiger:")
        for name, acc in target_val_acc_t.items():
            logger.write("{}: {:4.3f}".format(name, acc))
        return
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch < args.pretrain_epoch:
            ...
            if epoch == args.pretrain_epoch:
                ...
                student.load_state_dict(pretrained_dict, strict=False)
                teacher.load_state_dict(pretrained_dict, strict=False)
            train(train_source_iter, train_target_iter, student, teacher, D_img, style_net, criterion, con_criterion, im_criterion,
        ...
        if epoch < args.pretrain_epoch:
            ...
            target_val_acc_h = validate(val_target_loader_horse, student, criterion, visualize if args.debug else None, args)
            target_val_acc_t = validate(val_target_loader_tiger, student, criterion, visualize if args.debug else None, args)
        else:
            source_val_acc = validate(val_source_loader, teacher, criterion, None, args)
            target_val_acc = validate(val_target_loader, teacher, criterion, visualize if args.debug else None, args)
            target_val_acc_h = validate(val_target_loader_horse, teacher, criterion, visualize if args.debug else None, args)
            target_val_acc_t = validate(val_target_loader_tiger, teacher, criterion, visualize if args.debug else None, args)
        ...
        logger.write("Epoch: {} Source: {:4.3f} Target: {:4.3f} Horse: {:4.3f} Tiger: {:4.3f} Target(best): {:4.3f}".format(epoch, source_val_acc['all'], target_val_acc['all'], target_val_acc_h['all'], target_val_acc_t['all'], best_acc))
        logger.write("Source:")
...
def train1(train_source_iter, train_target_iter, student, teacher, D_img, style_net, criterion, con_criterion, im_criterion,
    ...
    for i in range(args.iters_per_epoch):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss_D_img_s = F.binary_cross_entropy_with_logits(D_img_s, torch.FloatTensor(D_img_s.data.size()).fill_(
                source_label).to(device))
            features_t = grad_reverse(features_t)
            D_img_t = D_img(features_t)
            loss_D_img_t = F.binary_cross_entropy_with_logits(D_img_t, torch.FloatTensor(D_img_t.data.size()).fill_(
                target_label).to(device))

            y_t_stu_recon = torch.zeros_like(y_t_stu).cuda()  # b, c, h, w
            for ind in range(x_t_stu.size(0)):
                _angle, _trans_x, _trans_y, _shear_x, _shear_y, _scale = angle[ind].item(), trans_x[ind].item(), \
                trans_y[ind].item(), shear_x[ind].item(), shear_y[ind].item(), scale[ind].item()
                temp = tF.affine(y_t_stu[ind], 0., translate=[_trans_x / ratio, _trans_y / ratio], shear=[0., 0.],
                                 scale=1.)
                temp = tF.affine(temp, _angle, translate=[0., 0.], shear=[0., 0.], scale=_scale)
            ...
            # loss_all = loss_s + args.lambda_c * loss_c  + loss_d
        loss_all = loss_s + args.lambda_c * loss_c + loss_D_img_s * 0.1 + loss_D_img_t * 0.1 + loss_d  # surreal—lsp best
        # loss_all = loss_s + args.lambda_c * loss_c + loss_D_img_s * 0.001 + loss_D_img_t * 0.001 + 0.2 * loss_d
...
def train(train_source_iter, train_target_iter, student, teacher, D_img, style_net, criterion, con_criterion,
          im_criterion,
          stu_optimizer, tea_optimizer, d_optimizer, epoch: int, visualize, args: argparse.Namespace):
    batch_time = AverageMeter('Time', ':4.2f')
    data_time = AverageMeter('Data', ':3.1f')
        ...
        with torch.no_grad():
            ...
            for ind in range(x_t_teas[0].size(0)):
                ...
                for _k in range(args.k):
                    ...
                    _angle, _trans_x, _trans_y, _shear_x, _shear_y, _scale = angle[ind].item(), trans_x[ind].item(), \
                    trans_y[ind].item(), shear_x[ind].item(), shear_y[ind].item(), scale[ind].item()
                    temp = tF.affine(y_t_teas[_k][ind], 0., translate=[_trans_x / ratio, _trans_y / ratio],
                                     shear=[0., 0.], scale=1.)
                    temp = tF.affine(temp, _angle, translate=[0., 0.], shear=[0., 0.], scale=_scale)
                    temp = tF.affine(temp, 0., translate=[0, 0], shear=[_shear_x, _shear_y], scale=1.)  # c, h, w
                    recons[_k] = temp  # c, h, w
            ...
            if args.occlude_rate > -1:  ###
                ...
                for _b in range(b):
                    if (conf_table[_b].sum() > 0 and np.random.rand() <= args.occlude_rate):
                        _angle, _trans_x, _trans_y, _shear_x, _shear_y, _scale = angle[_b].item(), trans_x[_b].item(), \
                        trans_y[_b].item(), shear_x[_b].item(), shear_y[_b].item(), scale[_b].item()
                        temp = tF.affine(x_t_stu[_b], 0., translate=[_trans_x / ratio, _trans_y / ratio],
                                         shear=[0., 0.], scale=1.)
                        temp = tF.affine(temp, _angle, translate=[0., 0.], shear=[0., 0.], scale=_scale)
                        temp = tF.affine(temp, 0., translate=[0., 0.], shear=[_shear_x, _shear_y], scale=1.)
                        ...
                        # warp it back
                        x_t_stu[_b] = tF.affine(temp, -_angle, translate=[-_trans_x / ratio, -_trans_y / ratio],
                                                shear=[-_shear_x, -_shear_y], scale=1. / _scale)
